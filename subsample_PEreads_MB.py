...
handler = logging.handlers.RotatingFileHandler('zz_subsample.log', maxBytes=2000000, backupCount=20)
...
def main(argv):
    ...
    try:
        if inputfile:
            ...
            missing_ids = []
            idList = []
            idList = read_mappingfile(inputfile , 1)
    ...
    with open(filename) as fh:
        for line in fh:
            ...
            if len(columns)> int(col_index):
                idList.append(columns[col_index])
            else:
                logger.error('Invalid column number is specified for read the input file')
                sys.exit(1)

    logger.info("IDs: {}".format(str(idList)))

    #sampleInfo = [line.rstrip('\n').split("\t") for line in fh]
    #idcol = 1
    #for line in sampleInfo:
    #    idList.extend(line[int(idcol-1)])
    #
...
def srun_script(scriptfile):
    ...
    cmd = "submit_job_array_slurm.sh " + scriptfile + " 1 | sbatch --mem=30000M";
    proc = subprocess.Popen(cmd, shell=True, stdout = subprocess.PIPE, stderr=subprocess.STDOUT)
