...
def main(args):
	...
	parser.add_argument('-t', '--tempdir', dest = 'tempdir', help = "Enter temp directory")
	...
	if not args.tempdir:
		logger.error("You must provide tempdir!")
		parser.exit(status=1, message = "You must tempdir!\n")
	else:
		tempdir = args.tempdir
	#job_status = check_job(job_id)

	filename = os.path.join(str(workdir), str(tempdir), "check_"+ str(job_id))
